...
approve-pairing: ## Approve a device pairing request. Usage: make approve-pairing NS=... [CLAW=instance] [REQUEST_ID=...]
	@if [ -z "$$REQUEST_ID" ]; then \
		...
		echo "Re-run with REQUEST_ID=<requestId> after reviewing the pending request."; \
		exit 1; \
	fi
	@case "$$REQUEST_ID" in *[!0-9A-Za-z-]*|'') echo "REQUEST_ID must contain only letters, digits, and hyphens."; exit 2;; esac
	@$(KUBECTL) exec -n $(NS) deployment/$(CLAW) -c gateway -- \
