...
|---|---|---|
| Operator | `gateway.*`, `models.providers`, `agents.defaults.models`, generated `agents.defaults.modelPolicy.allow` (7.2 only), `channels.<declared>`, `plugins.entries.<declared>` | Overwritten every restart |
| Operator → User | `agents.defaults.model.primary` | Set on first run, then preserved |
| Operator (skip if user overrides) | `agents.defaults.memorySearch` (legacy) or `memory.search` (7.2) | Auto-set from first embedding-capable credential; skipped entirely if the matching key is in `spec.config.raw` |
...
**Provider defaults**: The `knownProviders` registry in `claw_providers.go` centralizes all per-provider knowledge: credential type, domain, base path, wire format API identifier, Vertex AI SDK config, and model catalog. For known providers (`google`, `anthropic`, `openai`, `xai`, `openrouter`), non-Vertex credentials need only `provider` and `secretRef` — the operator infers `type`, `domain`, and base path. Explicit Vertex AI credentials instead require `type: gcp`. For `google` and `anthropic` (type `apiKey`), the auth header is also inferred. For `openai`, `xai`, and `openrouter`, the operator infers type `bearer` (standard `Authorization: Bearer` auth). `openrouter` also gets a `/api/v1` base path and a default model catalog with popular models from multiple upstream providers (e.g., `openai/gpt-5.5`, `anthropic/claude-sonnet-4-6`, `google/gemini-3.5-flash`). Providers not in the registry (e.g., custom providers) still work but require fully explicit configuration. Explicit values always take precedence as an escape hatch.

**Wire format API**: Each provider in `knownProviders` declares the OpenClaw wire format API it requires (e.g., `google-generative-ai` for Google, `anthropic-messages` for Anthropic, and `openai-responses` for xAI). `buildProviderEntry()` bakes the correct `api` field into every `models.providers` entry at generation time. Providers using the OpenClaw default (`openai-completions`) — like `openai` and unknown providers — omit the field entirely.

**Channel defaults**: known channels (telegram, discord, slack, whatsapp) have pre-configured domain, credential type, companion routes, and Secret-backed token config. Users only need to specify `channel` and `secretRef` — the operator infers proxy config and injects channel enablement into `operator.json`. Channel tokens are mounted into the gateway pod as operator-private env vars, and `operator.json` points OpenClaw at those env-backed token references rather than Kubernetes SecretRefs. This is intentionally weaker than the proxy-only LLM provider boundary until channel WebSocket/session auth can move behind the proxy. Explicit values always take precedence as an escape hatch.
...
**OpenAI provider IDs**: The 7.2 config generator normalizes OpenAI Codex aliases (`codex` and `openai-codex`) to `openai`; the legacy generator retains its `openai-codex` companion provider.
